...
const CircularProgessBar = ({percent, size = 3, strokeWidth = 0.25 , strokeColor = "green"}) => {\
  ...
  return (
    ...
    <svg width={`${size}vw`} height={`${size}vw`}>
      ...
      <circle r={`${radius}vw`} cx={`${size/2}vw`} cy={`${size/2}vw`} strokeWidth={`${strokeWidth}vw`} stroke={strokeColor} strokeDasharray={`${2*radius * Math.PI}vw`} fill='none' strokeDashoffset={`${2*radius* Math.PI- (percent / 100 * 2*radius * Math.PI)}vw`} transform="rotate(-90)" style={{transformOrigin: "center"}} strokeLinecap='round'/>
