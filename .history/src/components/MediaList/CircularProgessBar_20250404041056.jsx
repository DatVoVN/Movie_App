...
const CircularProgessBar = ({percent = 12, size = 3, strokeWidth = 0.25}) => {
  ...
  return (
    ...
    <svg width={`${size}vw`} height={`${size}vw`}>
      ...
      <circle r={`${radius}vw`} cx={`${size/2}vw`} cy={size/2} strokeWidth={strokeWidth} stroke='green' strokeDasharray={2*(radius) * Math.PI } fill='none' strokeDashoffset={125.65- (percent / 100 * 125.65) } transform="rotate(-90)" style={{transformOrigin: "center"}} strokeLinecap='round'/>
