...
const CircularProgessBar = ({percent = 12, size = 3, strokeWidth = 0.25}) => {
  ...
  return (
    ...
    <svg width={`${size}vw`} height={`${size}vw`}>
      ...
      <circle r={`${radius}vw`} cx={`${size/2}vw`} cy={`${size/2}vw`} strokeWidth={strokeWidth} stroke='green' strokeDasharray={2*radius * Math.PI } fill='none' strokeDashoffset={2*radius* Math.PI- (percent / 100 * 2*radius * Math.PI) } transform="rotate(-90)" style={{transformOrigin: "center"}} strokeLinecap='round'/>
